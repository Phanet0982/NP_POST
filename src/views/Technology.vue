  ...
  <div class="news-app-wrapper bg-light-subtle min-vh-100">
    <div class="container py-5 font-khmer">
      ...
      <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-end mb-5 pb-4 border-bottom">
        <div>
          <h2 class="fw-bold text-dark mb-1">ព័ត៌មានបច្ចេកវិទ្យា</h2>
          <p class="text-muted mb-0">អានព័ត៌មានថ្មីៗអំពីបច្ចេកវិទ្យា និងការច្នៃប្រឌិតថ្មីជុំវិញពិភពលោក</p>
        </div>
      ...
      <div v-if="filteredTech.length > 0" class="row g-4">
        <div v-for="item in filteredTech" :key="item.id" class="col-12 col-md-6 col-lg-4">
      ...
      <div v-else-if="isLoading" class="text-center py-5">
        ...
        <p class="mt-3 text-muted">កំពុងផ្ទុកព័ត៌មានបច្ចេកវិទ្យា...</p>
      </div>

      <div v-else class="text-center py-5">
        <i class="bi bi-cpu text-secondary opacity-25 display-1"></i>
...
// មុខងារ Filter សម្រាប់ Tech
const filteredTech = computed(() => {
  if (activeFilter.value === 'all') return techList.value;
  return techList.value.filter(item => 
...
const loadTechData = async () => {
  ...
  try {
    ...
    if (saved) {
      techList.value = saved.filter(item => 
        item.category === 'Tech' || item.category === 'បច្ចេកវិទ្យា'
      );
    }
  } catch (error) {
