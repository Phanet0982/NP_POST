  ...
  <div class="news-app-wrapper bg-light-subtle min-vh-100">
    <div class="container py-5 font-khmer">
            ...
            <div class="position-relative overflow-hidden">
              ...
              <template v-if="item.mediaType === 'video'">
                ...
                <div class="position-absolute top-50 start-50 translate-middle text-white">
                  <i class="bi bi-cpu-fill display-5 opacity-75"></i>
                </div>
              </template>
              <img v-else :src="item.media || item.image" class="card-img-top" style="height: 220px; object-fit: cover;">
              
              <div class="position-absolute top-0 start-0 p-3">
                <span class="badge bg-dark shadow px-3 border-0 text-white">បច្ចេកវិទ្យា</span>
              </div>
            </div>
      ...
      <div v-else class="text-center py-5">
        ...
        <p class="mt-3 text-muted">មិនទាន់មានព័ត៌មានបច្ចេកវិទ្យានៅឡើយទេ</p>
      </div>
...
const loadTechData = async () => {
  ...
  try {
    ...
    if (saved) {
      // ច្រោះយកតែប្រភេទ 'Tech' ឬ 'បច្ចេកវិទ្យា'
...
/* Global Sharp Edge Constraint - ជ្រុងស្រួច */
