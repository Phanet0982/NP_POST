    ...
    <div class="container py-5 font-khmer">
      ...
      <div v-if="!selectedNews">
        <div class="d-flex justify-content-between align-items-end mb-5 pb-4 border-bottom-news">
          <div>
            <h2 class="fw-bold text-dark mb-1 border-start border-primary border-4 ps-3">ព័ត៌មានសង្គម</h2>
            <p class="text-muted mb-0 ps-3">អានព្រឹត្តិការណ៍សង្គម និងការវិវត្តថ្មីៗក្នុងប្រទេស</p>
          </div>
        </div>
            ...
            <div @click="viewDetail(item)" class="card h-100 shadow hover-up bg-white">
              <div class="position-relative overflow-hidden">
                ...
                <div class="position-absolute top-0 start-0 p-3">
                  <span class="badge bg-primary shadow px-3">សង្គម</span>
                </div>
              </div>
      ...
      <div v-else class="row g-5 animate-fade-in">
        <div class="col-lg-8">
          ...
          <div class="d-flex align-items-center mb-3 border-bottom pb-2">
            ...
            <h6 class="fw-bold mb-0 text-uppercase">ប្រធានបទ៖ សង្គម</h6>
          </div>
          ...
          <div class="mb-4 py-3 d-flex align-items-center justify-content-between px-3 shadow bg-white">
            <div class="d-flex align-items-center gap-2">
              <img :src="`https://ui-avatars.com/api/?name=${selectedNews.author || 'Society'}&background=0d6efd&color=fff`" width="35" height="35">
              <span class="small text-muted">ដោយ៖ <strong class="text-dark">{{ selectedNews.author || 'អ្នករាយការណ៍ព័ត៌មាន' }}</strong></span>
            </div>
...
onMounted(() => {
  ...
  if (data) {
    ...
    filteredNews.value = allNews.value.filter(n => n.category === 'Society');
  }
});
</script>
