    ...
    <div v-if="!selectedNews">
      <div v-if="filteredNews.length > 0" class="row g-4">
        <div v-for="item in filteredNews" :key="item.id" class="col-md-6 col-lg-4">
          <div @click="viewDetail(item)" class="card h-100 border-0 shadow-sm society-card overflow-hidden cursor-pointer">
            <div class="position-relative overflow-hidden">
              <img :src="item.image" class="card-img-top society-img" :alt="item.title">
              <div class="position-absolute bottom-0 start-0 p-3 w-100 bg-gradient-dark">
                 <span class="badge bg-primary rounded-pill px-3">{{ item.badge || 'សង្គម' }}</span>
              </div>
            </div>
      ...
      <div v-else class="text-center py-5 bg-white rounded-4 shadow-sm border border-dashed">
        <div class="py-5">
          ...
          <h4 class="text-muted mt-3">មិនទាន់មានព័ត៌មានសង្គមនៅឡើយទេ</h4>
          <p class="text-muted small">សូមបន្ថែមព័ត៌មានក្នុង Dashboard ដោយជ្រើសរើសប្រភេទ "Society"</p>
        </div>
      </div>
    </div>
    ...
    <div v-else class="row g-5">
      <div class="col-lg-8">
        ...
        <div class="mb-4 border-bottom pb-3 text-muted small">
          ផ្នែក៖ <span class="fw-bold text-primary text-uppercase">ព័ត៌មានសង្គម</span> • {{ selectedNews.date }}
        </div>
        ...
        <div class="article-body fs-5 lh-lg mb-5 text-dark">
          ...
          <p v-else>ព័ត៌មានលម្អិតនៃអត្ថបទសង្គមនេះ នឹងត្រូវដាក់ជូនក្នុងពេលឆាប់ៗនេះ។</p>
        </div>

        <hr>
        <h4 class="fw-bold mb-4 mt-5">ព័ត៌មានសង្គមផ្សេងៗ</h4>
        <div class="row g-3">
          <div v-for="related in getRelated()" :key="'rel-'+related.id" class="col-md-4">
            <div @click="viewDetail(related)" class="cursor-pointer group">
              <img :src="related.image" class="img-fluid rounded-3 mb-2 society-img-small shadow-sm" style="height: 120px; width: 100%; object-fit: cover;">
      ...
      <div class="col-lg-4">
        <div class="sticky-top" style="top: 20px;">
          <h5 class="fw-bold pb-2 mb-4 border-bottom border-primary">ព័ត៌មានពេញនិយម</h5>
...
const selectedNews = ref(null); // Track selection
...
onMounted(() => {
  ...
  if (data) {
    ...
    filteredNews.value = allNews.filter(n => n.category === 'Society');
  }
});
</script>
...
.society-card:hover {
  ...
  border-bottom: 3px solid #0d6efd !important; /* Bootstrap Primary Blue */
}
...
.society-card:hover .society-img {
  transform: scale(1.1);
}

.society-img-small:hover {
  opacity: 0.85;
